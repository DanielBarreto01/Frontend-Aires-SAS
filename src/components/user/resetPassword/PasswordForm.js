...
const PasswordForm = () => {
    ...
    useEffect(() => {
        try {
            if (!localStorage.getItem('validateToken') || (JSON.parse(localStorage.getItem('validateToken')).token !== requestToken || !requestToken.trim().length === 0)) {
                ...
                axios.get(`/reset-password/validateStatusToken/${requestToken}`, config).then((response) => {
                    ...
                    console.log("validacion correcta", requestToken,"hola  ca", response.data);
                    setLoading(false);
                }).catch((error) => {
                    console.log(error);
